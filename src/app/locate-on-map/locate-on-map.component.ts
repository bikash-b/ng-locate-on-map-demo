...
import { MapPosition, MapCenter, PlacePhoto, ERRORS, MapConfig, InfoWindowEvent, MoreOptionEvent, ComponentLoadEvent } from './model';
...
export class LocateOnMapComponent implements OnInit {
  ...
  @ViewChild('parentComponent') parentComponentRef: ElementRef

  @Output() onOpenInfoWindow: EventEmitter<InfoWindowEvent> = new EventEmitter();
  @Output() onMoreOptionClick: EventEmitter<MoreOptionEvent> = new EventEmitter();
  @Output() onComponentLoad: EventEmitter<ComponentLoadEvent> = new EventEmitter();

  // Component class variables
  appLogo = require("./assets/apps.png");
  map: any;
  marker: any;
  autocomplete: any;
  photos: PlacePhoto[] = [];
  mapLoadingError: string[];
  moreOptions: string[] = ["Explore photos"];
  toggleOptions: any = {};
  toggleOption: boolean = false;
  togglePhotoGallery: boolean = false;
  ...
  ngOnInit() {
    this.onMapInit();
    let customOptions = this.mapConfig.moreOptions;
    if(customOptions){
  ...
  onMapInit(){
    ...
    window.navigator.geolocation.getCurrentPosition(location => {
      ...
      this.mapConfig.defaultPosition.center = latLng;
      console.log(this.mapConfig.defaultPosition);
      this.onMapLoad();
  ...
  getCompleteAddress(place){
    ...
    let displayDivider = (mapConfig.displayAddressOnInfoWindow || mapConfig.displayContactOnInfoWIndow || mapConfig.displayOpenHoursOnInfoWindow || mapConfig.displayRatingsOnInfoWindow || mapConfig.displayReviewOnInfoWindow) ? '</h2><hr style="border: 0.5px solid black;" />' : '';
    let content = `<h2 style='font-family: Arial;text-align:${this.mapConfig.infoWindowHeaderAlign}'>${place.name} ${displayDivider}`;
    let formattedAddredd = place.address_components;
    let completeAddress = this.mapConfig.displayAddressOnInfoWindow ? `<h2>Detail address</h2>` : '';
    let addressComp = '';
    let contactDetail = this.mapConfig.displayContactOnInfoWIndow ? `<div>
    ...
    let review = this.mapConfig.displayReviewOnInfoWindow ? this.getCompleteRating(place, true) : '';
    let ratings = this.mapConfig.displayRatingsOnInfoWindow ? this.getCompleteReview(place) : '';
    let openHours = this.mapConfig.displayOpenHoursOnInfoWindow ? this.getOpeningHours(place) : '';
  ...
  onAutoCompleteBoxLoad(){
    ...
    this.autocomplete.addListener('place_changed', () => {
      ...
      if(this.mapConfig.displayInfoWindow){
  ...
  toggleOptionContent(toggleOption: string){
    let key = toggleOption.toLowerCase().replace(' ', '_');
